...
- ガイドのworkflowはツール利用順ではなく、相談・合意・確認・引き渡し・記録など実際の業務全体を表す。すべての工程に実施内容と完了条件を持たせ、ツールslugは補助できる工程だけへ任意で関連付ける。
- 読み物の視覚要約は`GuideVisual`の型付きデータとして保持する。timeline、risk-map、pipeline、gatesを内容に応じて使い分け、本文と同じHTMLに見出し・説明・リストを含めて、画像だけに情報を閉じ込めない。
...
- トップは、サイトの価値説明、目的別カテゴリー、実践ガイド、ツール検索の順に配置する。内部リンクと説明本文をHTMLで保持し、見た目の再構成で検索向け文脈や到達経路を失わない。
