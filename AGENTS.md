...
企画候補は調査バッチ内だけで使う小文字アルファベット`a`、`b`、`c`…で識別する。開発承認時に次の未使用3桁台帳番号を付番し、それ以降のユーザーとの会話、仕様、開発、レビュー、公開管理では台帳番号を使う。候補記号を恒久IDにせず、未承認案へ台帳番号を予約しない。

実装・改善前に`PRODUCT_PLAN.md`の対象者、解決する面倒、無料版の核、有料候補、固有の確認事項を読む。調査で企画を変更した場合は、根拠とともに`PRODUCT_PLAN.md`を更新してから実装する。台帳番号は開発承認時の識別子であり、実際のリリース順とは別に扱う。
