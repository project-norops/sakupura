...
CSV・TSVファイルを読み込むツールは、初見の利用者が入力形式を理解できるよう、列名と入力例を含むテンプレートまたはサンプルファイルのダウンロードを用意する。文字コード変換のように固定の列構成がない場合は、その旨を画面に明記した上で確認用サンプルを用意する。

印刷・PDF保存機能は、サイトのヘッダー、フッター、入力フォーム、広告を印刷対象から除外する。表が複数ページに続く場合は見出し行の繰り返し、明細行の途中での改ページ防止、合計欄の分断防止を確認する。
